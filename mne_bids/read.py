...
def _assemble_hed_from_sidecar(events_dict, events_json_fname):
    ...
    In BIDS, HED tags can be specified per column value in the JSON sidecar
    (e.g. ``"face_type": {"HED": {"famous": "Def/Famous-face-cond"}}``).
    This function looks up each row's column values and concatenates
    the corresponding HED tags into one string per event.

    Parameters
    ----------
    events_dict : dict
        Column-oriented events data (as returned by ``_from_tsv``),
        already filtered for n/a onsets etc.
    events_json_fname : str | Path | None
        Path to the events JSON sidecar.

    Returns
    -------
    hed_strings : list of str | None
        One HED string per row, or None if no HED found in sidecar.
    """
...
def _handle_events_reading(
    ...
    if events_json_fname is not None:
        ...
        if sidecar_hed is not None:
            n_annot = len(annotations_info["onset"])
            if len(sidecar_hed) != n_annot:
                warn(
                    f"Sidecar HED row count ({len(sidecar_hed)}) does not "
                    f"match events ({n_annot}). Ignoring sidecar HED."
                )
                sidecar_hed = None
        if sidecar_hed is not None:
    ...
    if hed_strings is not None and hasattr(mne, "HEDAnnotations"):
        ...
        warn(
            "MNE-Python does not support HEDAnnotations. "
            "HED data stored in annotation extras."
        )
        # Store HED strings in extras as fallback
        extras = annotations_info["extras"]
        if extras is None:
            extras = [dict() for _ in range(len(hed_strings))]
        for extra, hed_str in zip(extras, hed_strings):
            extra["HED"] = hed_str
        annotations_info["extras"] = extras
    ...
    if len(annot_to_keep):
        if hasattr(mne, "HEDAnnotations") and isinstance(
            ...
            # TODO: simplify when MNE supports HEDAnnotations + Annotations
            # merging natively. Manual merge — pad with "n/a" for kept annots.
            existing_extras = getattr(raw.annotations, "extras", None)
            keep_extras = getattr(annot_to_keep, "extras", None)
            if existing_extras is not None or keep_extras is not None:
                n_existing = len(raw.annotations)
                n_keep = len(annot_to_keep)
                merged_extras = list(existing_extras or [dict()] * n_existing) + list(
                    keep_extras or [dict()] * n_keep
                )
            else:
                merged_extras = None
            combined = mne.HEDAnnotations(
                onset=np.concatenate([raw.annotations.onset, annot_to_keep.onset]),
                duration=np.concatenate(
                    [raw.annotations.duration, annot_to_keep.duration]
                ),
                description=np.concatenate(
                    [raw.annotations.description, annot_to_keep.description]
                ),
                hed_string=list(raw.annotations.hed_string)
                + ["n/a"] * len(annot_to_keep),
